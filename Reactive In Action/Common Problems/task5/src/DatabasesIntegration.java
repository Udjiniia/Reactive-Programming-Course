...
public class DatabasesIntegration {
							...
							.flatMap((tuple) -> {
								...
								if (fileDbResult.error() == null && oracleDbResult.error() == null) {
									...
									if (fileDbResult.error() != null && oracleDbResult.error() != null) {
										...
										Mono<Void> voidMono =
												oracleDb.rollbackTransaction(oracleDbResult.transactionId());
										Mono<Void> voidMono1 =
												fileDb.rollbackTransaction(fileDbResult.transactionId());
										return voidMono.then(voidMono1);
									}
									else if (oracleDbResult.error() != null) {
    ...
    static Mono<Result> dbWriteInTransaction(DatabaseApi db, Flux<Integer> dataSource) {
        return Mono
				.usingWhen(
						db.<Integer>open().retryWhen(Retry.max(1).filter(t -> t instanceof IllegalAccessError)),
